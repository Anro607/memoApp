...
class CustomMemoApp(QMainWindow):
    ...
    def __init__(self):
        ...
        self.setWindowIcon(QIcon(os.path.join(self.base_path, 'icon.icns')))
...
def get_base_path():
    ...
    Handles both development (script) and PyInstaller (bundled .exe/.app) modes.
    For macOS .app bundles, navigates up from Contents/MacOS/ to the .app directory.
    """
    if getattr(sys, 'frozen', False):
        ...
        if '.app/Contents/MacOS' in exe_path:
            # Navigate up 3 levels: Contents/MacOS/ -> Contents/ -> .app/ -> parent dir
